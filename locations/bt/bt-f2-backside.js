// ── locations/bt/f2-backside.js ────────────────────────────────────
export default [
  {
    id: 'bt-f2-backside-1',
    name: 'Level 2 - Backside 1',
    caption: 'BT - Level 2 - Backside 1',
    panorama: './images/bt/f2-backside/bt-f2-backside-1.jpg',
    thumbnail: './images/bt/f2-backside/thumbs/bt-f2-backside-1.jpg',
    defaultYaw: '0deg',
    defaultPitch: '0deg',
    defaultZoomLvl: 0,
    links: [
      {
        nodeId: 'bt-f2-backside-2',
        position: { yaw: '94.48deg', pitch: '0deg' },
        name: 'Go to Level 2 - Backside 2',
      },
      {
        nodeId: 'bt-stairs-back-1',
        position: { yaw: '8.29deg', pitch: '0deg' },
        name: 'Go to Stairs - Back 1',
      },
      {
        nodeId: 'bt-f2-backside-3',
        position: { yaw: '277.88deg', pitch: '0deg' },
        name: 'Go to Level 2 - Backside 3',
      },
      {
        nodeId: 'bt-f2-reading-10',
        position: { yaw: '186.90deg', pitch: '0deg' },
        name: 'Go to Level 2 - Reading Area 10',
      },
    ],
    markers: [],
    data: {
      floor: 'f2',
      tags: ['backside'],
    },
  },
  {
    id: 'bt-f2-backside-2',
    name: 'Level 2 - Backside 2',
    caption: 'BT - Level 2 - Backside 2',
    panorama: './images/bt/f2-backside/bt-f2-backside-2.jpg',
    thumbnail: './images/bt/f2-backside/thumbs/bt-f2-backside-2.jpg',
    defaultYaw: '0deg',
    defaultPitch: '0deg',
    defaultZoomLvl: 0,
    links: [
      {
        nodeId: 'bt-f2-backside-1',
        position: { yaw: '275.84deg', pitch: '0deg' },
        name: 'Go to Level 2 - Backside 1',
      },
    ],
    markers: [],
    data: {
      floor: 'f2',
      tags: ['backside'],
    },
  },
  {
    id: 'bt-f2-backside-3',
    name: 'Level 2 - Backside 3',
    caption: 'BT - Level 2 - Backside 3',
    panorama: './images/bt/f2-backside/bt-f2-backside-3.jpg',
    thumbnail: './images/bt/f2-backside/thumbs/bt-f2-backside-3.jpg',
    defaultYaw: '0deg',
    defaultPitch: '0deg',
    defaultZoomLvl: 0,
    links: [
      {
        nodeId: 'bt-f2-backside-1',
        position: { yaw: '86.75deg', pitch: '0deg' },
        name: 'Go to Level 2 - Backside 1',
      },
      {
        nodeId: 'bt-f2-surau-1',
        position: { yaw: '13.25deg', pitch: '0deg' },
        name: 'Go to Level 2 - Surau 1',
      },
    ],
    markers: [],
    data: {
      floor: 'f2',
      tags: ['backside'],
    },
  },
];

